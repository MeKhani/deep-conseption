...
class InGramEntityLayer(nn.Module):
    ...
    def forward(self, emb_ent, emb_rel, triplets): 
        ...
        ent_freq = torch.zeros((num_ent, )).index_add(dim = 0, index = tail_idxs, \
                                                             source = torch.ones_like(tail_idxs, dtype = torch.float)).unsqueeze(dim = 1)
        ...
        #                                                             include_self = False)
        attn_val_max = torch.zeros((num_ent, self.num_head, 1)).scatter_reduce(dim = 0, \
                                                                    index = scatter_idx.to(torch.int64), \
                                                                    src = attn_val_raw, reduce = 'amax', \
                                                                    include_self = False)
...
class InGramRelationLayer(nn.Module):
    ...
    def forward(self, emb_rel, relation_triplets):
        ...
        #                                                             src = attn_val_raw, reduce = 'amax', \
        #                                                             include_self = False)
        attn_val_max = torch.zeros((num_rel, self.num_head, 1)).scatter_reduce(dim = 0, \
                                                                    index = scatter_idx.to(torch.int64), \
                                                                    src = attn_val_raw, reduce = 'amax', \
                                                                    include_self = False)
        ...
        output = torch.zeros((num_rel, self.num_head, self.dim_hid_rel)).index_add(dim = 0, \
                                                                                            index = head_idxs, 
                                                                                            source = beta * self.aggr_proj(emb_rel[tail_idxs]).view(-1, self.num_head, self.dim_hid_rel))
