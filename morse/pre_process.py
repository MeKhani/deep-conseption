...
def data2pkl(data_name):
    ...
    # print(f" vaidation triples : {valid_tri}")
    file.close()

    test_tri = []
    file = open('./dataset/{}/test.txt'.format(data_name))
    test_tri.extend([l.strip().split("\t") for l in file.readlines()])
    # print(f" test triples : {test_tri}")
    file.close()

    train_tri, fix_rel_reidx, ent_reidx = reidx(train_tri)
    # print(f"train_tri {len(train_tri)}")
    # print(f"fix_rel_reidx  {len(fix_rel_reidx)}")
    # print(f"ent_reidx  {len(ent_reidx)}")
